...
if REGULATOR_TF:
    ...
    df[df["TF.Gene.Symbol"]==GENE].sort_values(by=["Importance"], ascending=False).dropna().iloc[:TOP_K].to_csv("top_k_regulators_and_regulated_genes/"+file_name, index=False)
    
if REGULATED_TARGET:
    file_name = "_".join(["top", str(TOP_K), "genes", "that", "regulate", GENE+".csv"]) 
    df[df["Target.Gene.Symbol"]==GENE].sort_values(by=["Importance"], ascending=False).dropna().iloc[:TOP_K].to_csv("top_k_regulators_and_regulated_genes/"+file_name, index=False) 
